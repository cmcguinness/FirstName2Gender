...
getNamesSimple <- function () {
        ...
        males <- integer(0)
        females <- integer(0)
        names2index <- new.env(hash=T, parent=emptyenv())
        
        print("This takes quite a while. I'll print out each year as it's processing")
        # We look at names from 1945 to 2013.  The data goes back a lot further
        # than that, though, so 
        for (year in 1945:2014) {
                print(year)
